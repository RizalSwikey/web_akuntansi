  ...
  if (addIncomeBtn && addIncomeModal && incomeForm) {
    ...
    incomeForm.addEventListener("submit", function (event) {
      ...
      if (dataType === "usaha") {
        ...
        const newRowHTML = `<div class="data-row grid grid-cols-10 gap-4 items-center" data-total="${total}"><button class="remove-row-btn text-slate-400 hover:text-red-500 -mr-2"><svg class="w-5 h-5 pointer-events-none" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.134-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.067-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg></button><div class="col-span-3 p-3">${productName}</div><div class="col-span-2 p-3">${quantity}</div><div class="col-span-2 p-3">${formatRupiah(
          price
        ...
        const newRowHTML = `<div class="data-row grid grid-cols-10 gap-4 items-center" data-total="${total}"><button class="remove-row-btn text-slate-400 hover:text-red-500 -mr-2"><svg class="w-5 h-5 pointer-events-none" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.134-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.067-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg></button><div class="col-span-7 p-3">${productName}</div><div class="col-span-2 p-3 font-semibold">${formatRupiah(
          total
        )}</div></div>`;
        document
    ...
    document
      ...
      .addEventListener("click", function (event) {
        ...
        if (removeBtn) {
          ...
          const isUsaha = row.parentElement.id === "pendapatan-usaha-rows";
          row.remove();
          updateGrandTotal(isUsaha ? "usaha" : "lain");
        }
      });
  }
});
